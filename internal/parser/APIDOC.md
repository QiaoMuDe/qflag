...
**边界处理:**
- 遇到"--"或单独的"-"停止扫描, 后面视为位置参数
- 遇到子命令名停止扫描, 后续标志由子命令处理

**扫描延续性（与标准库的差异）:**

预扫描对普通位置参数会**继续向后扫描**以捕错，而非在首个位置参数处停止（这与标准库 `flag.Parse` 不同——
标准库在首个位置参数处即停止解析）。对于**无子命令**的命令，出现在位置参数之后以 `-` 开头的 token 会被
预扫描报为未知标志，而实际解析会将其作为位置参数保留、不报错。例如 `myapp --port 8080 file.txt --typo`：

- 预扫描：报告未知标志 `--typo`；
- 实际解析（标准库）：`--typo` 作为位置参数被保留，不报错。

此为预扫描「宁可多报、不漏捕」的既定取舍。若需完全对齐标准库语义，可在 `checkUnknownFlags` 中将非子命令
位置参数处的 `continue` 改为 `break`。
