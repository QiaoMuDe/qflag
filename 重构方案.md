...
graph TD
    %% 外部API层
    CmdAPI[cmd/cmd.go<br/>外部API适配器] --> Types[internal/types<br/>共享类型定义]
    
    %% 功能子包层
    Parser[internal/parser<br/>解析器子包] --> Types
    Registry[internal/registry<br/>注册表子包] --> Types
    SubCmd[internal/subcmd<br/>子命令管理子包] --> Types
    Builtin[internal/builtin<br/>内置标志子包] --> Types
    Help[internal/help<br/>帮助生成子包] --> Types
    Validator[internal/validator<br/>通用验证子包] --> Types
    
    %% 外部依赖
    Types --> Flags[flags/<br/>标志类型包]
    Types --> StdFlag[Go标准库<br/>flag包]
    ...
    %% 保持现有的文件
    CmdAPI --> Basic[basic.go<br/>基础标志方法]
    CmdAPI --> Numeric[numeric.go<br/>数值标志方法]
    CmdAPI --> Collection[collection.go<br/>集合标志方法]
    CmdAPI --> Time[time.go<br/>时间标志方法]
    CmdAPI --> Completion[completion.go<br/>自动补全]
    CmdAPI --> BashComp[bash_completion.go<br/>Bash补全]
    CmdAPI --> PwshComp[pwsh_completion.go<br/>PowerShell补全]
    
    %% 样式定义
    classDef coreType fill:#e1f5fe,stroke:#01579b,stroke-width:2px,color:#000
    classDef funcPkg fill:#f3e5f5,stroke:#4a148c,stroke-width:2px,color:#000
    classDef apiLayer fill:#e8f5e8,stroke:#1b5e20,stroke-width:2px,color:#000
    classDef external fill:#fff3e0,stroke:#e65100,stroke-width:2px,color:#000
    classDef existing fill:#fce4ec,stroke:#880e4f,stroke-width:2px,color:#000
    
    %% 应用样式
    class Types coreType
    class Parser,Registry,SubCmd,Builtin,Help,Validator funcPkg
    class CmdAPI apiLayer
    class Flags,StdFlag external
    class Basic,Numeric,Collection,Time,Completion,BashComp,PwshComp existing
```
...
func ParseArgs(ctx *types.CmdContext, args []string, parseSubcmds bool) error {
    ...
    // 一次性更新参数，减少锁持有时间
    ctx.Mutex.Lock()
    ctx.Args = append(ctx.Args, parsedArgs...)
    argsToProcess := make([]string, len(ctx.Args))
    copy(argsToProcess, ctx.Args)
    ctx.Mutex.Unlock()
    
    // 解析子命令（使用复制的参数，避免再次加锁）
    if parseSubcmds && len(argsToProcess) > 0 {
...
func ParseSubCommandSafe(ctx *types.CmdContext, args []string) error {
    ...
    if exists {
        remainingArgs := args[1:] // 直接使用传入的参数切片
