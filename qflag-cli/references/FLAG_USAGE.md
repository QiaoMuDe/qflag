...
QFlag 遵循标准库 `flag` 包语义，遇到下列 token 时**停止标志扫描**，其后的所有参数一律按**位置参数**处理（即使以 `-` 开头）：

```bash
# 使用 "--"：终止标志解析，之后的内容全部视为位置参数
myapp -- local-value

# 使用单独的 "-"：同样终止标志解析，"-" 本身视为位置参数
myapp - -some-value
```

> 注：标志的值允许以 `-` 开头（如 `--port -8080`、`--timeout -30s`），此时 `-8080`/`-30s` 会作为该标志的值，不会被误判为标志。
...
枚举标志的帮助描述文本可使用便捷函数 `EnumHelp` 生成（支持 "值" 与 "值: 描述" 两种形态）：

```go
help := qflag.EnumHelp("运行模式", []string{"debug: 调试模式", "release: 发布模式"}, "\t")
```

- 对齐按终端显示宽度计算（全角/CJK 占 2 列），中英混排时 `]` 与描述列仍能对齐
- 空值选项（纯空白或冒号开头）会被自动跳过
- 值（value）本身不能包含冒号，冒号仅作值与描述的分隔符
