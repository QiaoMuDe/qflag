...
func (f *BaseFlag[T]) Type() FlagType
    Type 返回标志类型 注意：具体标志类型需要重写此方法返回正确的FlagType
...
type IP4Flag struct {
    ...
    IP4Flag IPv4地址类型标志结构体 继承BaseFlag[string]泛型结构体,实现Flag接口
...
type IP6Flag struct {
    ...
    IP6Flag IPv6地址类型标志结构体 继承BaseFlag[string]泛型结构体,实现Flag接口
...
type URLFlag struct {
    ...
    URLFlag URL类型标志结构体 继承BaseFlag[string]泛型结构体,实现Flag接口
