...
返回标志类型。注意：具体标志类型需要重写此方法返回正确的 FlagType。
...
通过标志名称查找标志元数据。参数 `name` 可以是长名称（如 "help"）或短名称（如 "h"）。
...
设置是否忽略键的大小写。`enable` 为 `true` 时，所有键将转换为小写进行存储和比较。
...
检查切片是否包含指定元素。当切片未设置值时，将使用默认值进行检查。
...
实现 flag.Value 接口，返回当前时间的字符串表示。加锁保证 outputFormat 和 value 的并发安全访问。
