...
func TestBuiltinFlags(t *testing.T) {
	...
	t.Run("root command version flags", func(t *testing.T) {
		// 创建带有版本信息的根命令
		rootCmd1 := NewCmd("test", "t", flag.ContinueOnError)
		rootCmd1.SetExitOnBuiltinFlags(false)
		...
		if !rootCmd1.ctx.BuiltinFlags.Version.Get() {
	...
	t.Run("ParseFlagsOnly handles builtin flags", func(t *testing.T) {
		...
		if !parseFlagsCmd.ctx.BuiltinFlags.Version.Get() {
...
func TestGenerateHelpInfo_BasicCommand(t *testing.T) {
	...
	helpInfo := help.GenerateHelp(cmd.ctx)
...
func TestBindHelpFlag(t *testing.T) {
	...
	if _, ok := cmd.ctx.FlagRegistry.GetByName(flags.HelpFlagName); !ok {
		t.Error("帮助标志应该已注册")
	}

	// 当短帮助标志名存在时，检查该标志是否已注册，若未注册则报错。
	_, ok := cmd.ctx.FlagRegistry.GetByName(flags.HelpFlagShortName)
...
func TestCmd_Args(t *testing.T) {
	...
	cmd := &Cmd{ctx: &types.CmdContext{Args: args}}
	result := cmd.Args()
	// 检查长度
...
func TestCmd_Arg(t *testing.T) {
	cmd := &Cmd{ctx: &types.CmdContext{Args: []string{"arg0", "arg1", "arg2"}}}
	tests := []struct {
		name string
		i    int
		want string
