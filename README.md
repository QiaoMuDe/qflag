...
- **标志依赖** - 支持标志依赖关系, 定义触发标志与目标标志之间的约束（互斥或必需）
...
func main() {
    ...
    cmd.Root.AddMutexGroup(formatGroup)
...
func main() {
    ...
    // 添加普通必需组 - 所有标志都必须设置
    cmd.Root.AddRequiredGroup("connection", []string{"host", "port"}, false)
    cmd.Root.AddRequiredGroup("auth", []string{"username", "password"}, false)
    
    // 添加条件性必需组 - 如果使用其中一个则必须同时使用
    cmd.Root.AddRequiredGroup("optional-features", []string{"feature-a", "feature-b"}, true)
...
func main() {
    ...
    // 添加条件性必需组 - 如果使用其中一个则必须同时使用
    qflag.Root.AddRequiredGroup("database", []string{"dbhost", "dbport", "dbuser", "dbpass"}, true)
...
标志依赖关系是QFlag提供的一种灵活的标志验证方式，它允许您定义标志之间的依赖约束。当触发标志被设置时，目标标志会受到相应的约束（互斥或必需）。
...
func main() {
    // 配置全局根命令
    qflag.Root.SetDesc("服务器配置工具")
    
    // 创建标志
    remoteFlag := qflag.Root.Bool("remote", "r", "使用远程模式", false)
    localPathFlag := qflag.Root.String("local-path", "l", "本地路径", "")
    ...
    if remoteFlag.Get() {
        fmt.Println("使用远程模式")
    } else if localPathFlag.IsSet() {
        fmt.Printf("本地路径: %s\n", localPathFlag.Get())
    }
    
    if sslFlag.Get() {
        fmt.Printf("SSL证书: %s, 密钥: %s\n", certFlag.Get(), keyFlag.Get())
    }
}
```
...
- **DepMutex (互斥依赖)**: 当触发标志被设置时，目标标志不能被设置
  - 示例：远程模式与本地路径互斥
  - 示例：配置文件模式与手动配置参数互斥
  
- **DepRequired (必需依赖)**: 当触发标志被设置时，目标标志必须被设置
  - 示例：SSL模式需要证书和密钥
  - 示例：数据库模式需要连接字符串

**使用场景:**

1. **模式互斥**: 当应用程序支持多种运行模式，每种模式使用不同的配置参数时
2. **功能依赖**: 当启用某个功能时，需要同时提供相关的配置参数时
3. **条件约束**: 当某些配置参数之间存在条件性的约束关系时
...
// 选项配置
qflag.ApplyOpts(opts)   // 应用选项到全局根命令
```
